...
export default class HeaderDashboard extends Component {
  ...
  render() {
    ...
    return (
      <div className={classnames(className, styles.header, { [styles.fixedHeader]: isFixed })}>
        ...
        {withCreateBtn.linkTo && (
          <Link to={withCreateBtn.linkTo}>
            <Button className={styles.btnCreate} type="primary">
              ...
              <span className={styles.btnText}> {withCreateBtn.name || t('Create')} </span>
            </Button>
          </Link>
        )}
        {withCreateBtn.onClick && (
          <Button className={styles.btnCreate} type="primary" onClick={withCreateBtn.onClick}>
            <Icon name="add" size={20} type="white" />
            <span className={styles.btnText}> {withCreateBtn.name || t('Create')} </span>
          </Button>
        )}
